...
class FieldAwareFactorizationMachine(nn.Module):
    ...
    def __init__(self, field_dims: np.ndarray, embed_dim: int):
        ...
        # self.attention = MultiHeadAttention(d_feat=embed_dim)
    ...
    def forward(self, x: torch.Tensor):
        ...
        # xs = [self.attention(xs[i]) for i in range(self.num_fields)]
        ix = list()
...
class _Deep_FieldAwareFactorizationMachineModel(nn.Module):

    def __init__(self, field_dims: np.ndarray, user_field_idx, item_field_idx, embed_dim, mlp_dims, num_layers, dropout):
        super().__init__()
        self.user_field_idx = user_field_idx
        self.item_field_idx = item_field_idx
        field_dims = field_dims[:-2]
        self.embedding = FeaturesEmbedding(field_dims, embed_dim)
        self.embed_output_dim = len(field_dims) * embed_dim
        ...
        self.attention = MultiHeadAttention(d_feat=embed_dim)
        self.cd_linear = nn.Linear(2, 1, bias=True)
        self.linear = FeaturesLinear(field_dims)
    def forward(self, x: torch.Tensor):
        """
        :param x: Long tensor of size ``(batch_size, num_fields)``
        ...
        ffm_term = torch.sum(torch.sum(self.ffm(x[:,:8]), dim=1), dim=1, keepdim=True)
        x = self.embedding(x[:,:8])
        # cn_term = self.cn(x.view(-1, self.embed_output_dim))
        x = self.attention(x)
        # linear = self.linear(x)
        fm_term = self.fm(x)
        # x = self.mlp(cn_term)
        # x = self.cd_linear(torch.tensor(ub_mean,dtype=torch.float))
        
        y = 2.5*fm_term + 1.5*ffm_term
...
class _MyTransformer(nn.Module):
    def __init__(self, field_dims, embed_dim):
        ...
        self.attention = MultiHeadAttention(d_feat=embed_dim)
        self.embedding = FeaturesEmbedding(field_dims, embed_dim)
        self.linear = nn.Linear(self.embed_output_dim, 1, bias=True)
        
        
    def forward(self, x):
        embed_x = self.embedding(x)
        attention = self.attention(embed_x)
        y = self.linear(attention.view(-1, self.embed_output_dim))
        return y.squeeze(1)
    
    
class MultiHeadAttention(nn.Module): #27,9
    def __init__(self,d_feat,n_head=9,USE_BIAS=True,dropout=0.1,device=None):
        """

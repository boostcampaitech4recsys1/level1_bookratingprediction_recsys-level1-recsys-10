...
def main(args):
    ...
    if args.MODEL in ('FM', 'NCF', 'WDN', 'DCN'):
        predicts = model.predict(data['test_dataloader'])
    if args.MODEL in ('FFM'):
        predicts, predicts1, predicts2 = model.predict(data['test_dataloader'], data['test_short_dataloader'], data['short_list'], data['n_test'])
    elif args.MODEL=='CNN_FM':
        predicts  = model.predict(data['test_dataloader'])
    elif args.MODEL=='DeepCoNN':
        predicts  = model.predict(data['test_dataloader'])
    ...
    submission1 = pd.read_csv(args.DATA_PATH + 'sample_submission.csv')
    submission2 = pd.read_csv(args.DATA_PATH + 'sample_submission.csv')

    if args.MODEL in ('FM', 'NCF', 'WDN', 'DCN', 'CNN_FM', 'DeepCoNN'):
        submission['rating'] = predicts
    elif args.MODEL in ('FFM'):
        submission['rating'] = predicts
        submission1['rating'] = predicts1
        submission2['rating'] = predicts2       
    ...
    submission.to_csv('submit/{}_{}_1.csv'.format(save_time, args.MODEL), index=False)
    submission1.to_csv('submit/{}_{}_2.csv'.format(save_time, args.MODEL), index=False)
    submission2.to_csv('submit/{}_{}_3.csv'.format(save_time, args.MODEL), index=False)
...
if __name__ == "__main__":
    ...
    arg('--EPOCHS1', type=int, default=10, help='Epoch 수를 조정할 수 있습니다.')
    arg('--EPOCHS2', type=int, default=10, help='Epoch 수를 조정할 수 있습니다.')
